...
def exact_match_score(opt, prediction, ground_truth, all_ground_truth, index, length, gt_number):
    try:
        length = int(np.ceil(length / opt.per_gpu_batch_size))

        if index is not None and gt_number==0:
            print('Batch: [{}/{}]  |  Prediction: {:12}  |  Ground Truth: {}'.format(index, length, prediction, all_ground_truth))
    except Exception:
        pass
